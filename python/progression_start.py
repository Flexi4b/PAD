    ...
    if result:
        ...
        if result2:
            ...
            if result3:
                ...
                html += '            <a class="admin" href="../html/challenge1/challenge1_complete.html">Admin</a> \n'
                html += '        </footer> \n'
                html += ' \n'
                html += '    </div> \n'
                html += '</body> \n'
                ...
                query4 = "INSERT INTO `pad`.`Challenge` (`speler_id`,`start_time`,`challenge_id`) VALUES ((SELECT `speler_id` FROM `pad`.`Speler` WHERE `username` = '{fuser}' AND `password` = '{fpassword}'), CURRENT_TIMESTAMP(), '1')".format(fuser=username, fpassword=password)
