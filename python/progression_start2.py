...
def application(environ, start_response):
    ...
    if result:
        query2 = "SELECT speler_id FROM Score WHERE speler_id = (SELECT speler_id FROM Speler WHERE username = '{fusername}' AND password = '{fpassword}')".format(fusername=username, fpassword=password)
        dbcursor.execute(query2)
        result2 = dbcursor.fetchall()
        if result2:
            query3 = "SELECT start_time FROM Challenge WHERE speler_id = (SELECT speler_id FROM Speler WHERE username = '{fusername}' AND password = '{fpassword}') AND challenge_id = 2".format(fusername=username, fpassword=password)
            ...
            if result3:
                ...
                html += '            <a class= "selected" href="../html/challenge2/cryptobank.html">Home</a><hr> <!-- Hier link ik dus naar andere html bestanden en geef ik een display naam-->\n'
                html += '            <a class= "selected" href="../html/challenge2/banklogin.html">Inloggen</a><hr> <!-- Hier link ik dus naar andere html bestanden en geef ik een display naam-->\n'
                html += '        </div>\n'
                html += '\n'
                ...
                query4 = "INSERT INTO `pad`.`Challenge` (`speler_id`,`start_time`,`challenge_id`) VALUES ((SELECT `speler_id` FROM `pad`.`Speler` WHERE `username` = '{fuser}' AND `password` = '{fpassword}'), CURRENT_TIMESTAMP(), '2')".format(fuser=username, fpassword=password)
